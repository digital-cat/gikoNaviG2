...
function setHash(){
 ...
 for(var i=setEndIdx;i<l;i++){
  ...
  for(var j=0;j<ddAnc.length;j++){
   try{ddAnc[j].href}catch(err){continue} // IE7だと何故かhrefが取得出来ないA要素がある
   var ancType=checkAnchor(ddAnc[j].href);
   if      (ancType==0){
    continue;
   }else if(ancType==1){
...
function setSearchColor(dt){
 ...
 // anc
 var dd=dt.nextSibling; if(!dd){return} // footerのdtから呼ばれると次のddは不存在
 ddAnc=dd.getElementsByTagName("A");
 for(var i=ddAnc.length;i--;){
  var h=tohan(ddAnc[i].innerText);var tNum=h.replace(/\D/g,"");
  if(refHash[tNum]){
   var refResult=refHash[tNum].split(" ");
   if     (refResult.length>=threshold6[0]){ddAnc[i].style.color=threshold6[1];}
   else if(refResult.length>=threshold5[0]){ddAnc[i].style.color=threshold5[1];}
   else if(refResult.length>=threshold4[0]){ddAnc[i].style.color=threshold4[1];}
  }
 }
}
//==========名前・ID検索等
...
function returnFound(type,color){
 ...
 // 対象オブジェクト（検索の時は重複排除）
 if(type=="res"){
  cHash=foundRes;numArray=cHash[color[0]];
 }else{
  cHash=highlight;numArray=cHash[color[0]];
 }
 // 表示方法
 var display=new Array(["none","POP"],["inline","LST"]);
 if(searchView=="resPopup" || numArray.length==0  || numArray.length>=listLimit){display=display.reverse();}
 // ポップアップ
...
function searchWord(){
 if(!word){return} word=word.replace(/(^\x20+|\x20+$)/g,"");
 ...
 if(expression=="regExp" || expression=="default" || (expression.match(/multi/) && !word.match(/\s/))){ // AND/ORで一単語ならdefaultで検索
  ...
  if(expression=="regExp"){
   ...
   var reg=new RegExp(regWord,"g"); var tmpArr=range.text.match(reg);
   if(!tmpArr || !tmpArr.length){tmpArr=new Array();}
   var tmpHash={}; for(var i=tmpArr.length;i--;){tmpHash[tmpArr[i]]=true} for(var i in tmpHash){matchs.push(i)} // 正規表現でのマッチパターンの重複を排除
   if(!matchs || !matchs.length){matchs[0]=word} // 検索結果0or1なら、配列作成
  }else{matchs[0]=word}
  // ハイライト→重複排除処理
  var tmpHash={};
  for(var i=0;i<matchs.length;i++){
   var tmp=setHighlight(matchs[i]);
   color[0]=tmp[0];  // 単色なので常にcolor[0]に入れる
   for(var j=tmp[1].length;j--;){tmpHash[tmp[1][j]]=true;} // 同一レス内部での複数マッチを排除
  }
  createHash(highlight,color[0],"",matchs[0]);
  for(var i in tmpHash){highlight[color[0]].push(i);}
  highlight[color[0]].sort(sortByInt);
  highlight[color[0]].word=word;
 }else{
  // 検索語指定
  var matchs=word.split(" "); var tmpArr=new Array();
  // ハイライト→重複処理
  for(var i=0;i<matchs.length;i++){
   var tmp=setHighlight(matchs[i]);
   color.unshift(tmp[0]);
   var tmpHash={};for(var j=tmp[1].length;j--;){tmpHash[tmp[1][j]]=true} for(var j in tmpHash){tmpArr.push(j);} // 同一レス内部での複数マッチを排除
   highlight[color[0]].word=matchs[i];
  }
  color.reverse();tmpArr.sort(sortByInt);
  createHash(highlight,color[0],"",matchs[0]); // 一旦初期化
  var tmpHash={};var judge=(expression=="multiAND") ? matchs.length+"<=tmpHash[i]" : "1";
  for(var i=0;i<tmpArr.length;i++){if(tmpHash[tmpArr[i]]){tmpHash[tmpArr[i]]++}else{tmpHash[tmpArr[i]]=1}}
  for(var i in tmpHash){if(eval(judge)){highlight[color[0]].push(i)}} // 検索結果はhighlight[color[0]]に入れる
 }
 // 結果表示
...
function setHighlight(reg){
 // カラー選択
 var c=selectColor("highlight"); if(!c){return} var nums=[];
 // 検索
 while(range.findText(reg)){
  ...
  if(dt.parentElement.className!="refResult"){nums.push(num)} // 逆参照内のDTなら回避
...
function copyText(target){
 ...
 else if(target=="mail"){textarea.value=obj.childNodes[2].nodeValue+"\n";}
...
//==========sort関数
function sortByInt(x,y){return(x-y)}
